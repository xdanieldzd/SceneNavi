...
namespace SceneNavi
{
	...
	public partial class MainForm : Form
	{
		...
		private void CreateSceneTree()
		{
			...
			if (!individualFileMode)
			{
				...
				foreach (ISceneTableEntry ste in rom.Scenes)
				{
					...
					{
						if (!(ste.GetSceneHeaders()[0].Commands.FirstOrDefault(x => x.Command == HeaderLoader.CommandTypeIDs.Rooms) is HeaderCommands.Rooms rooms)) continue;
			...
			else
			{
				...
				if (tempScene.GetSceneHeaders()[0].Commands.FirstOrDefault(x => x.Command == HeaderLoader.CommandTypeIDs.Rooms) is HeaderCommands.Rooms rooms)
				{
		...
		private void RefreshCurrentData()
		{
			...
			if (currentScene != null)
			{
				...
				if (rooms != null)
				{
					...
					{
						new XMLActorDefinitionReader.Definition.Item.Option() { Description = "(All Rooms)", Value = 0x3F }
					};
					foreach (HeaderCommands.Rooms.RoomInfoClass ric in rooms.RoomInformation)
			...
			if (currentScene != null && currentScene.GetActiveCollision() != null && currentScene.GetActiveCollision().Waterboxes.Count > 0)
			{
				...
				{
					new HeaderCommands.Collision.Waterbox()
				};
		...
		private void RefreshWaypointPathList(HeaderCommands.Waypoints wp)
		{
			...
			{
				new HeaderCommands.Waypoints.PathHeader()
			};
			...
			if (dgv.Columns[e.ColumnIndex].Name == "Number")
			{
				if (e != null && e.Value != null && e.DesiredType.Equals(typeof(ushort)))
				{
					...
					if (ushort.TryParse((ishex ? str.Substring(2) : str), (ishex ? System.Globalization.NumberStyles.AllowHexSpecifier : System.Globalization.NumberStyles.None),
						System.Globalization.CultureInfo.InvariantCulture, out ushort val))
					{
						e.Value = val;
		...
		private void RefreshRoomActorList()
		{
			...
			{
				new HeaderCommands.Actors.Entry()
			};
		...
		private void cbTransitions_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			}), (rooms?.RoomInformation), individualFileMode);
		}
		...
		private void RefreshCollisionPolyAndTypeLists()
		{
			...
			{
				new HeaderCommands.Collision.PolygonType()
			};
			...
			{
				new HeaderCommands.Collision.Polygon()
			};
		...
		private void enableAntiAliasingToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			{
				GL.GetInteger(GetPName.MaxSamples, out int samples);
		...
		private void aboutToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			MessageBox.Show(string.Format("{0} {1}\n\nScene/room actor editor for The Legend of Zelda: Ocarina of Time\n\nWritten {2} by xdaniel - http://github.com/xdanieldzd/", Program.AppNameVer, buildString, yearString),
