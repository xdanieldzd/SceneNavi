...
namespace SceneNavi.ROMHandler
{
	public class ROMHandler
	{
		...
		private void FindDMATable()
		{
			for (int i = 0; i < MinROMSize; i += 8)
			{
				uint tmp1 = Endian.SwapUInt32(BitConverter.ToUInt32(Data, i));
				uint tmp2 = Endian.SwapUInt32(BitConverter.ToUInt32(Data, i + 4));

				if (tmp1 == 0x00000000 && tmp2 == 0x00001060)
				{
					uint tmp3 = Endian.SwapUInt32(BitConverter.ToUInt32(Data, i + 0x10));
					if (tmp3 == tmp2)
					{
						DMATableAddress = i;
						return;
